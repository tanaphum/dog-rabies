...
plot(out$time/365, out[,"s1FI"]
     ,col = "1",
     xlab="Time (Year)",ylab="Number of dogs", type = "l",
     ylim=c(0,500),xlim = c(0,20),main = "Type of dogs")
lines(out$time/365, out[,"s2FI"], col="2")
lines(out$time/365, out[,"s3FI"], col="3")
lines(out$time/365, out[,"s1MI"], col="4")
lines(out$time/365, out[,"s2MI"], col="5")
lines(out$time/365, out[,"s3MI"], col="6")

#Name graph  
#title("Dog dynamics", "Indoor Female example")

#Add a legend (https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/legend)
legend("topright", legend=c("FI Pups", "FI Adults", "FI Older"),
...
plot(times, out[,31]
     ,col = "31",
     xlab="Time (days)",ylab="Number of dogs", type = "l",
     ylim=c(0,500),xlim = c(0,(365*14)),main = "Type of dogs")
...
#out 43-45 Female, 46-48 Male outdoor vaccination
plot(times, out[,43]
     ,col = "43",
     xlab="Time (days)",ylab="Number of dogs", type = "l",
     ylim=c(0,500),xlim = c(0,(365*20)),main = "Type of dogs")
lines(times, out[,44], col="44")
lines(times, out[,45], col="45")
lines(times, out[,46], col="46")
lines(times, out[,47], col="47")
lines(times, out[,48], col="48")

#out 49-51 Female, 52-54 Male stray vaccination
plot(times, out[,49]
     ,col = "49",
     xlab="Time (days)",ylab="Number of dogs", type = "l",
     ylim=c(0,500),xlim = c(0,(365*20)),main = "Type of dogs")
lines(times, out[,50], col="50")
lines(times, out[,51], col="51")
lines(times, out[,52], col="52")
lines(times, out[,53], col="53")
lines(times, out[,54], col="54")
